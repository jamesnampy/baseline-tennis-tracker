...
import { activePointEvents, numberedPointEvents, pointDetailsMap, pointGameNumber, pointSetNumber, voidedPointIds } from "./scoring.ts";
...
export function buildExportBundle(matches: MatchRecord[] | MatchRecord, players: PlayerProfile[] = [], mappings: IdentityMapping[] = [], anonymize = false, report?: { match: MatchRecord; options: CoachReportOptions }) {
  ...
  const files: Record<string, string> = {
    ...
    "points.csv": csv(["match_id","point_id","point_number","set_number","game_number","event_sequence","timestamp","server","receiver","winner","serve_result","score_before","score_after","outcome","rally_range"], points.map(({match,point,pointNumber,details}) => [match.id,point.pointGroupId,pointNumber,pointSetNumber(point),pointGameNumber(point),point.sequence,point.timestamp,point.payload.server,point.payload.receiver,point.payload.winner,point.payload.serveResult,point.payload.scoreBefore,point.payload.scoreAfter,details?.outcome,details?.rallyRange])),
    ...
    "games.csv": csv(["match_id","event_id","sequence","set_number","game_number","winner","server","hold","games","tiebreak","voided"], selected.flatMap((m) => { const voided=voidedPointIds(m.events); return m.events.filter((e) => e.type === "game_completed").map((e) => [m.id,e.id,e.sequence,e.payload.setNumber,e.payload.gameNumber,e.payload.winner,e.payload.server,e.payload.hold,e.payload.games,e.payload.tiebreak,e.pointGroupId?voided.has(e.pointGroupId):false]); })),
    "sets.csv": csv(["match_id","event_id","sequence","set_number","winner","games","tiebreak","is_match_tiebreak","sets_won"], selected.flatMap((m) => m.events.filter((e) => e.type === "set_completed").map((e) => [m.id,e.id,e.sequence,e.payload.setNumber,e.payload.winner,e.payload.games,e.payload.tiebreak,e.payload.isMatchTiebreak,e.payload.setsWon]))),
    "match_status.csv": csv(["match_id","event_id","sequence","timestamp","type","winner","reason","retiring_player","score"], selected.flatMap((m) => m.events.filter((e) => e.type === "match_completed" || e.type === "player_retired").map((e) => e.type === "match_completed" ? [m.id,e.id,e.sequence,e.timestamp,e.type,e.payload.winner,e.payload.reason,undefined,e.payload.score] : [m.id,e.id,e.sequence,e.timestamp,e.type,e.payload.winner,"retirement",e.payload.player,e.payload.score]))),
    "strategy_reviews.csv": csv(["match_id","event_id","sequence","request_id","requested_at","generated_at","cutoff_sequence","provider","model","prompt_version","coverage","stale","evidence_count"], selected.flatMap((m) => { const stale=staleStrategyEventIds(m); return m.events.filter((e) => e.type === "strategy_generated").map((e) => [m.id,e.id,e.sequence,e.payload.requestId,e.payload.requestedAt,e.timestamp,e.payload.cutoffSequence,e.payload.provider,e.payload.model,e.payload.promptVersion,e.payload.coverage,stale.has(e.id),e.payload.evidence.length]); })),
...
/**
 * Requirements section 14: a review is stale once a point it analyzed is undone.
 * A `point_undone` recorded after the review's cutoff voids a point group whose
 * events were inside that cutoff, so the review no longer describes the match.
 */
export function staleStrategyEventIds(match:MatchRecord):Set<string>{
  const groupSequence=new Map<string,number>();
  for(const event of match.events)if("pointGroupId"in event&&event.pointGroupId&&!groupSequence.has(event.pointGroupId))groupSequence.set(event.pointGroupId,event.sequence);
  const undoneAt=match.events.filter((event)=>event.type==="point_undone").map((event)=>({sequence:event.sequence,pointSequence:groupSequence.get(event.payload.pointGroupId)??Infinity}));
  const stale=new Set<string>();
  for(const event of match.events){
    if(event.type!=="strategy_generated")continue;
    if(undoneAt.some((undo)=>undo.sequence>event.sequence&&undo.pointSequence<=event.payload.cutoffSequence))stale.add(event.id);
  }
  return stale;
}
