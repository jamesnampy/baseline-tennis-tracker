...
const shotLabels: Record<string, string> = {
  ...
  backhand: "Backhand", neither: "Neither",
};
...
function MatchTracker({ match, setMatch, saved, onExit }: { match: MatchRecord; setMatch: (match: MatchRecord) => void; saved: boolean; onExit: () => void }) {
  ...
  useEffect(() => { if (stage === "details" && details.rallyRange && details.finalStroke && details.shotType && details.advancedShotType) { const timer = window.setTimeout(() => finishDetails(), 180); return () => window.clearTimeout(timer); } }, [details, stage]); // eslint-disable-line react-hooks/exhaustive-deps
